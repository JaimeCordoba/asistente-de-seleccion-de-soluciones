...
class SistemaSeleccionSoluciones {
  ...
  async cargarDatos() {
    const respuestaEntradas = await fetch("./data/entradas.json");
    this.entradas = await respuestaEntradas.json();

    const respuestaSalidas = await fetch("./data/salidas.json");
    this.salidas = await respuestaSalidas.json();

    const respuestaReglas = await fetch("./data/reglas.json");
  ...
  identificarCamposObligatorios() {
    ...
    for (const reglasSalida of Object.values(this.reglas)) {
      ...
      Object.keys(condicionesObligatorias).forEach((variable) => this.camposObligatorios.add(variable));

      // Campos de reglas de inclusión
      const reglasIncluye = reglasSalida.reglas_inclusion || [];
      reglasIncluye.forEach((regla) => {
        this.extraerVariablesDeRegla(regla).forEach((variable) => this.camposObligatorios.add(variable));
      });
  ...
  determinarCamposRelevantes(salidasPosibles) {
    ...
    Object.keys(this.valoresEntradas).forEach((campo) => camposRelevantes.add(campo));
    ...
    for (const salida of salidasPosibles) {
      ...
      (reglasSalida.reglas_inclusion || []).forEach((regla) => {
        this.extraerVariablesDeRegla(regla).forEach((campo) => camposRelevantes.add(campo));
      });
      ...
      (reglasSalida.reglas_blandas || []).forEach((regla) => {
        ...
        if (regla.condicion) {
          this.extraerVariablesDeRegla(regla.condicion).forEach((campo) => camposRelevantes.add(campo));
        }
      });
    }
